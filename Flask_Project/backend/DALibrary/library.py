...
class Library:
    ...
    def add_book(self, title: str, author: str, published_year: int, genre: str) -> str:
        """Method that add book to library.

        :param title: Book title
        :param author: Book author
        :param published_year: Published year
        :param genre: Book genre
        :return: result description string
        """
        book = Book(title, author, published_year, genre)
    ...
    def give_books(self, book_id_list: list, reader_id: int) -> str:
    ...
    def return_books(self, book_id_list: list) -> str:
        ...
        msg = ''
        for book_id in book_id_list:
            msg += f'{self.return_book(book_id)}'
    ...
    def get_reader_by_id(self, user_id: int) -> Any:
        """Method that find book by book identifier

        :param user_id: unique book object identifier
        :return: User object if found else return None
        """
        return self.__storage.get_user_by_id(user_id)

    def add_reader(self, first_name: str, second_name: str, age: int, email: str, psw_hash: str) -> str:
        """Method that add reader to library.

        :param first_name: User first_name
        :param second_name: User second name
        :param age: User age
        :param email: User email
        :param psw_hash: User password hash
        :return: result description string
        """
        reader = Reader(first_name, second_name, age)

        reader_id = self.__storage.save_reader(reader)

        return self.add_user(email, psw_hash, reader_id)
    ...
    def add_user(self, email: str, psw_hash: str, reader_id) -> str:
        """Method taht add user to DB
        ...
        :return: string
        """
        user = User(email, psw_hash, reader_id)

        return self.__storage.save_user(user)
    ...
    def get_user_by_email(self, email: str) -> User:
        ...
        :return: User object
        """
        return self.__storage.get_user_by_email(email)
